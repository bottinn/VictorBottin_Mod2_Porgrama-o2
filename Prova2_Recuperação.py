eventos = {
        "Matemática": {"capacidade": 50, "inscritos": []},
        "Fotográfia": {"capacidade": 30, "inscritos": []},
    }
alunos = ["Antônio Cabeção", "Bruno", "Carla"]
...
def SelecionarEvento(eventos):
    if not eventos:
        ...
        return None
    lista_eventos = list(eventos.items())
    print("Eventos disponíveis:")
    for idx, (titulo, dados) in enumerate(lista_eventos, start=1):
        vagas = dados["capacidade"] - len(dados["inscritos"])
        print(f"{idx}. {titulo} | Vagas Restantes: {vagas}")
    try:
        escolha = int(input("Digite o número do evento para inscrição: "))
        if 1 <= escolha <= len(lista_eventos):
            return lista_eventos[escolha - 1]  # Retorna (título, dados do evento)
        else:
...
def CadastrarAluno(alunos, eventos):
    ...
    selecionado = SelecionarEvento(eventos)
    if selecionado:
        titulo, evento = selecionado
        return InscreverAlunoNoEvento(NomeAluno, evento)
    else:
        alunos.remove(NomeAluno)
...
def ExibirEventos(eventos):
    ...
    for titulo, dados in eventos.items():
        vagas_restantes = dados["capacidade"] - len(dados["inscritos"])
        lista_eventos.append(f"Evento: {titulo} | Capacidade: {dados['capacidade']} | Vagas Restantes: {vagas_restantes}")
